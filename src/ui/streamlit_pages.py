...
from src.output import ArtifactGenerator
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if api.llm_credentials_configured(config):
            st.success("LLM: Configured for provider %s" % config.llm_provider)
        else:
...
def render_settings_page():
    ...
    # API Key status
    if api.llm_credentials_configured(config):
        st.success("LLM credentials are configured for provider: %s" % config.llm_provider)
        if config.llm_provider == "openrouter" and config.openrouter_api_key and st.checkbox("Show API Key"):
            st.code(config.openrouter_api_key[:10] + "..." + config.openrouter_api_key[-4:])
    else:
        st.warning("LLM credentials are not set for provider: %s" % config.llm_provider)
        st.markdown("""
